...
def trips_to_and_back(gifts):
    ...
    for cur_index in gift_index:
        ...
        if gifts['TripId'].loc[cur_index] == -1:
            if (cur_weight + gifts['Weight'].loc[cur_index]) <= 990:
                ...
                # split trip to the way to and way back
                tmp_split = split_trip(gifts, cur_trip)
                tmp_oneway = split_trip(gifts, cur_trip)
                if weighted_trip_length(tmp_split[['Latitude', 'Longitude']], list(tmp_split['Weight'])) < \
                   weighted_trip_length(tmp_oneway[['Latitude', 'Longitude']], list(tmp_oneway['Weight'])):
                    print 'split'
                    trips.append(tmp_split)
                else:
                    print 'one way'
                    trips.append(tmp_oneway)
    ...
    trips.append(split_trip(gifts, cur_trip))
    gifts = pd.concat(trips, axis=0)
    # print gifts
    return gifts
...
def split_trip(gifts, cur_trip):
    """
    Split trip between the way south and north
    """
    trip = gifts[gifts['TripId'] == cur_trip]
    trip = trip.sort('Longitude', ascending=True)
    n_gifs_trip = trip.shape[0]

    trip_south = trip.iloc[:(n_gifs_trip/2)]
    trip_south = trip_south.sort('Latitude', ascending=False)

    trip_north = trip.iloc[(n_gifs_trip/2):]
    trip_north = trip_north.sort('Latitude', ascending=True)

    trip = pd.concat([trip_south, trip_north])
    return trip
...
gift_trips.to_csv('cluster_north_south_round_trips.csv')
...
# Continent clustering 0-3, batch = 5 optimization: